<template>
	<div class="confirmOrder">
		...
		<!-- 6.8修改测试  测试好像不成功？？？重新试一下？？
		6.8 11:27 测试两个平台是否有冲突 gitee是没有冲突的
		-->
		...
		<div class="content">
			...
			<div class="section-address">
				...
				<div class="address-body">
					<ul>
						<!-- 样式的判断  遍历的地址与当前选择的地址相同  默认是第一个  要实现点击不同地址修改不同的样式-->
						<!-- 怎么实现它呢？ -->
						<li :class="item.id == confirmAddress ? 'in-section' : ''" v-for="item in address"
							:key="item.id">
							<h2>{{item.name}}</h2>
	...
	export default {
		...
		data() {
			return {
				...
				// 地址列表
				//模拟的数据，真实情况应该是把它设置为空列表，然后在组件创建的第一时间，向后台发请求，带着他的id，获取到地址的数据，然后更新，在对应的界面那里去遍历显示
				address: [{
						id: 1,
						name: "陈同学",
						phone: "13580018623",
		...
		computed: {
			// 结算的商品数量; 结算商品总计; 结算商品信息   getCheckNum  这里拿到前面的路由被勾选的商品的信息  getCheckGoods  拿到被勾选的商品
