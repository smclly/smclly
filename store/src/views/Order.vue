		...
		<div class="order-content" v-if="orders.length>0">
			<div class="content" v-for="(item,index) in orders" :key="index">
				...
				<ul>
					<!-- 我的订单表头 -->
					<li class="order-info">
						<!-- 直接拿第一个的订单， 反正一个订单不管几个商品。订单编号都一样的  随便拿 默认第一个-->
						<div class="order-id">订单编号: {{item[0].order_id}}</div>
						<div class="order-time">订单时间: {{item[0].order_time | dateFormat}}</div>
						<!-- 涉及到一个格式化的问题吧  把时间戳转换成时间-->
						<!-- 过滤器应该被添加在 JavaScript 表达式的尾部，由“管道”符号指示：  <div v-bind:id="rawId | formatId"></div> -->
					</li>
					<li class="header">
				...
				<div class="order-bar">
					<div class="order-bar-left">
						<span class="order-total">
							...
							<!-- total[index].totalNum  数组里是对象，根据索引值找 对应的数据 -->
						</span>
					</div>
...
<script>
	export default {
		...
		activated() {
			// 获取订单数据  不是挂载后做事，是每次进入组件都要更新订单数据
			this.$axios
				.post("/api/user/order/getOrder", {
					user_id: this.$store.getters.getUser.user_id //另类的计算属性
				})
				.then(res => {
		...
		//在当前组件使用自定义的过滤器  当全局过滤器和局部过滤器重名时，会采用局部过滤器。
		/* filters: { 
