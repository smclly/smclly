		...
		<div class="content" v-if="getShoppingCart.length>0">
			<ul>
				...
				<li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
					...
					<div class="pro-num">
						<!-- 手动修改商品数量时触发 -->
						<!-- 饿了么组件自带的方法  :value="item.num" -->
						<el-input-number size="small" :value="item.num"
							@change="handleChange($event,index,item.productID)" :min="1" :max="item.maxNum">
						</el-input-number>
					</div>
			...
			<div class="cart-bar">
				...
				<div class="cart-bar-right">
					...
					<!-- 一般在这里，应该是把勾选的商品id带着给到订单结算界面  这里不是的，直接通过购物车搞事情   通过切换勾选状态，给vuex的数据做更新   -->
				</div>
			</div>
			<!-- 购物车底部导航条END -->
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			/* 方法里面导入...mapActions  还是和vuex通信，省事吗？ */
			...mapActions(["updateShoppingCart", "deleteShoppingCart", "checkAll"]),
			
			// 修改商品数量的时候调用该函数
			handleChange(currentValue, key, productID) {  //e 省略了$e 对吧  失去焦点的值 遍历索引， 商品id
				...
				// 向后端发起更新购物车的数据库信息请求  根据商品id，修改数量
				this.$axios
			...
			//更改购物车数据
			checkChange(val, key) {  //是否勾选 t/f  和循环索引  拿到遍历时的id， 我觉得会好一点
				// 更新vuex中购物车商品是否勾选的状态
				//console.log(val,key)
				this.updateShoppingCart({
					key: key,
			...
			// 向后端发起删除购物车的数据库信息请求
			deleteItem(e, id, productID) {  //目标 商品编号，商品id
				this.$axios
					...
					.then(res => {
						switch (res.data.code) {
							case "001":
								// “001” 删除成功
								// 更新vuex状态
								this.deleteShoppingCart(id);  //删除vuex购物车的id 更新
								// 提示删除成功信息
								this.notifySucceed(res.data.msg);
								break;
							default:
